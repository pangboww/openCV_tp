...
QcvVideoCapture::~QcvVideoCapture()
{
	...
	if (updateThread != NULL)
	{
		...
		{
			mutex.lock();
			// qDebug() << "QcvVideoCapture::~QcvVideoCapture: lock";
		}
		lockLevel++;
	}
	...
	if (updateThread != NULL)
	{
		...
		if (lockLevel == 0)
		{
			// qDebug() << "QcvVideoCapture::~QcvVideoCapture: unlock";
			mutex.unlock();
		}
						   ...
						   const unsigned int height)
{
	if (updateThread != NULL)
	{
		...
		{
			mutex.lock();
			// qDebug() << "QcvVideoCapture::open(" << deviceId << "...): lock";
		}
		lockLevel++;
	}
	...
	if (grabbed)
	{
		...
		// emit
		// message changed already emitted by grabInterval()
	...
	if (updateThread != NULL)
	{
		...
		if (lockLevel == 0)
		{
			// qDebug() << "QcvVideoCapture::open(" << deviceId << "...): unlock";
			mutex.unlock();
		}
	}
						   ...
						   const unsigned int height)
{
	...
	if (updateThread != NULL)
	{
		...
		{
			mutex.lock();
			// qDebug() << "QcvVideoCapture::open(" << fileName << "...): lock";
		}
		lockLevel++;
	}
	...
	if (grabbed)
	{
		...
		// emit changes
		// messageChanged already emitted by grabInterval
		emit imageChanged(&imageDisplay);
	...
	if (updateThread != NULL)
	{
		...
		if(lockLevel == 0)
		{
			// qDebug() << "QcvVideoCapture::open(" << filename << "...): unlock";
			mutex.unlock();
		}
	}
							  ...
							  const unsigned int height)
{
	if ((updateThread != NULL))
	{
		...
		{
			mutex.lock();
			// qDebug("QcvVideoCapture::setSize(%d, %d) locked", width, height);
		}
		lockLevel++;
	}
	...
	// qDebug("QcvVideoCapture::setSize(%d, %d)", width, height);
	...
	if (updateThread != NULL)
	{
		...
		if (lockLevel == 0)
		{
			// qDebug("QcvVideoCapture::setSize unlocked");
			mutex.unlock();
		}
	}
...
void QcvVideoCapture::setFlipVideo(const bool flipVideo)
{
	...
	if (updateThread != NULL)
	{
		...
		{
			mutex.lock();
			// qDebug() << "QcvVideoCapture::setFlipVideo(): lock";
		}
		lockLevel++;
	}
	...
	if (updateThread != NULL)
	{
		...
		if (lockLevel == 0)
		{
			// qDebug() << "QcvVideoCapture::setFlipVideo(): unlock";
			mutex.unlock();
		}
	}
...
void QcvVideoCapture::setGray(const bool grayConversion)
{
	...
	if (updateThread != NULL)
	{
		...
		{
			mutex.lock();
			// qDebug() << "QcvVideoCapture::setGray(): lock";
		}
		lockLevel++;
	}
	...
	if (updateThread != NULL)
	{
		...
		{
			mutex.unlock();
			// qDebug() << "QcvVideoCapture::setGray(): unlock";
		}
	}
...
int QcvVideoCapture::grabInterval(const QString & message)
{
	...
	// For instance it does not work wirh linuxes equipped with V4L
	// --------------------------------------------------------------------
#ifndef Q_OS_LINUX
...
//	qDebug("framerate direct query = %f", frameRate);
...
void QcvVideoCapture::update()
{
	...
	if (updateThread != NULL)
	{
		if (skip)
		{
			...
			// qDebug() << "QcvVideoCapture::update trylock"
			// 		 << (locked ? "granted" : "failed");
			if (locked)
			{
				lockLevel++;
			}
		...
		else
		{
			...
			{
				mutex.lock();
				// qDebug() << "QcvVideoCapture::update lock";
			}
			lockLevel++;
		}
	}
	...
	if (capture.isOpened() && locked)
	{
		...
		if (!image.data) // captured image has no data
		{
			...
			{
				// We'll try to rewinds the file back to frame 0
		...
		if (updateThread != NULL)
		{
			...
			if (lockLevel == 0)
			{
				// qDebug() << "QcvVideoCapture::update unlock";
				mutex.unlock();
			}
		}
	...
	else
	{
		...
		// qDebug() << "Capture skipped an image";
	}
}
